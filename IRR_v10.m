function [h,ph_dev,s_en,Rat,status,high_sol,r_save] = IRR_v10(L,f_s,f_p,s_v,p_l_v,...
...
maximize delta*trace(UU1*X)-(1-delta)*(trace(Cpass*X))
subject to
    for i=1:f_sample
...
%             trace(P(:,:,i)*X) <= 0;       % Use this for phase constraints.
            real(trace(Lk(:,:,i)*X)) <= 0;  % Use this for group delay constraints.
            real(trace(Uk(:,:,i)*X)) >= 0;  % Use this for group delay constraints.
        end
    end
...
if sum(nan_check(:)) == 0 && isnan(cvx_optval) == 0
    ...
    plot(diag(S1)/sum(diag(S1)),'.-');axis tight;ylim([-1 6]);xlim([-1 order_r]);grid;
...
r_save = ratio_old;
for ii=1:aim_num
[X,high_sol,status,Rat,rfig,figs1] = optimizer_v5(ii,10,U1,S1,C,Cpass,Clowconstraint,M1_s,...
    M2_s,err,delSing,p_add,fgrid,plbul,Lk,Uk,order_r,irr_c,ratio_old,...
...
vv = vv';
fff=Ffir*vv.';
figure;stem(tgrid,fff);xlabel('Time (seconds)');ylabel('Magnitude');
title(['Designed filter in time domain with # of coefficients = ',num2str(L)]);

figure;plot(fgrid/Fs,20*log10(abs(Ffir)));hold on;plot(fgrid/Fs,10*log10(M1),'r');hold on;
...
% Save the results
save(['IRR_v10-L-',num2str(L),'-f_s-',num2str(f_s),'-f_p-',num2str(f_p),...
